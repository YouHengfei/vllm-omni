...
class VibeVoiceRuntimeConfig:
    negative_kv_cache_memory_bytes: int = 2 * 1024**3
    ...
    def from_vllm_config(cls, vllm_config: Any) -> VibeVoiceRuntimeConfig:
        ...
        raw = (
            additional_config.get("vibevoice_runtime_config")
        ...
        for key, value in raw.items():
            ...
            if isinstance(value, bool):
                raise ValueError(
                    f"VibeVoice runtime config {key} must be an integer, not bool."
                )
            try:
                values[key] = int(value)
            except (TypeError, ValueError) as exc:
                raise ValueError(
                    f"VibeVoice runtime config {key} must be an integer, "
                    f"got {value!r}."
                ) from exc
        ...
        if result.negative_kv_cache_memory_bytes <= 0:
            raise ValueError(
                "negative_kv_cache_memory_bytes must be positive."
            )
        if result.negative_kv_activation_margin_bytes < 0:
            raise ValueError(
                "negative_kv_activation_margin_bytes must be non-negative."
            )
        return result
