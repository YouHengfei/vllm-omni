...
@lru_cache(maxsize=1)
def _tokenizer():
    return AutoTokenizer.from_pretrained(_TOKENIZER, trust_remote_code=False)
...
def _extract_audio(multimodal_output: dict) -> torch.Tensor:
    assert isinstance(multimodal_output, (dict, Mapping))
    audio = multimodal_output.get("audio")
    if audio is None:
        audio = multimodal_output.get("model_outputs")
    assert audio is not None
    if isinstance(audio, list):
        chunks = [torch.as_tensor(item).float().cpu().reshape(-1) for item in audio if item is not None]
        assert chunks
...
def _assert_valid_audio(output) -> None:
    audio = _extract_audio(output.outputs[0].multimodal_output)
    assert audio.numel() > 0
    assert audio.numel() % 3_200 == 0
    assert torch.isfinite(audio).all()
    assert 0.1 < audio.numel() / _SAMPLE_RATE < 60.0
