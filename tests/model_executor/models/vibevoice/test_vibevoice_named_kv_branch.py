...
@pytest.mark.parametrize(
    ("raw", "message"),
    [
        ({"future_key": True}, "Unknown VibeVoice runtime config keys: ['future_key']"),
        ("not-a-mapping", "vibevoice_runtime_config must be a mapping."),
    ],
)
def test_runtime_config_rejects_unrecognized_schema(raw: object, message: str) -> None:
    config = SimpleNamespace(additional_config={"vibevoice_runtime_config": raw})
...
def test_runtime_config_parses_diffusion_graph_bool() -> None:
    ...
    assert default.cuda_graph_capture_failure_fatal is False
    ...
    assert disabled.cuda_graph_capture_failure_fatal is True
